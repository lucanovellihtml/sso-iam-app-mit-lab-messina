(function (window) {
  ...
  function spid_populate(spid_elements) {
    ...
    if (spid_elements.length > 0) {
      fetch(queryURL)
        ...
        .then(function (idpListUrl) {
          idpListUrl = idpListUrl.sort(() => Math.random() - 0.5)
          for (var u = 0; u < spid_elements.length; u++) {
            for (var i = 0; i < idpListUrl.length; i++) { spid_addIdpEntry(idpListUrl[i], spid_elements[u]); }

          }
        })
        .catch(function (error) {
          console.log('Error during fetch: ' + error.message);
          idpListUrl.sort(() => Math.random() - 0.5)
          for (var u = 0; u < spid_elements.length; u++) {
            for (var i = 0; i < idpListUrl.length; i++) { spid_addIdpEntry(idpListUrl[i], spid_elements[u]); }
          }
        });
  ...
  function spid_addIdpEntry(data, element) {
    ...
    if (element.id.indexOf('post-cittadino') !== -1) {
      li.innerHTML = `<button class="idp-button-idp-logo" onclick='this.form.action="${spInitiatedBaseUrl.replace("@entity_id@", data['entity_id'])}";' name="${data['organization_name']}" type="submit"><span class="spid-sr-only">${data['organization_name']}</span><img class="spid-idp-button-logo" src="${data['logo_uri']}" alt="${data['organization_name']}" /></button></li>`
    };
    if (element.id.indexOf('post-azienda') !== -1) {
      li.innerHTML = `<button class="idp-button-idp-logo" onclick='this.form.action="${spInitiatedBaseUrlAzienda.replace("@entity_id@", data['entity_id'])}";' name="${data['organization_name']}" type="submit"><span class="spid-sr-only">${data['organization_name']}</span><img class="spid-idp-button-logo" src="${data['logo_uri']}" alt="${data['organization_name']}" /></button></li>`
    };
